; Minimal SSD1306 text demo using software I2C on GPIO pins.
;
; Wiring:
...
; positions the write cursor at the top-left, and writes "OK" using
; a tiny built-in 5x7-style bitmap.
;
; Visible LED result:
; - 0x01 -> success
; - 0x02 -> address NACK
...
setup:
    ...
    ; Use page addressing so B0/00/10 is enough to position the cursor.
    ldi #0x20
    mov rb, ra
    call write_command
    mov rd, rb
    ...
    ; Resume RAM display, normal pixels, display on.
    ldi #0xA4
    mov rb, ra
    call write_command
    mov rd, rb
    ...
    ; Clear all pages first so stale GDDRAM contents do not look random.
    ldi #0xB0
    mov rb, ra
    call clear_page
    mov rd, rb
    ...
    ; Start a contiguous data stream.
